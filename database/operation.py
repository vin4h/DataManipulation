...
class Operation:
    ...
    def create_product_formatted_table(self):
        self.db.connect()
        query = "CREATE TABLE IF NOT EXISTS product_formatted (id SERIAL PRIMARY KEY, id_product int, converted_price_brl float, converted_price_usd float, converted_price_eur float, quote_brl float, quote_usd float, quote_eur float, created_at timestamp DEFAULT now())"
        self.db.execute(query)
    ...
    def insert_product_formatted_table(self, id_product: int, converted_price_brl: float, converted_price_usd: float, converted_price_eur: float, quote_usd: float, quote_eur: float, quote_brl: float):
        try:
            self.db.connect()
            query = "INSERT INTO product_formatted (name, price, country) values (%s, %s, %s, %s, %s, %s, %s)"
